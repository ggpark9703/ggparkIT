...
var FindId = function()
{
    ...
    this.action = function(sMallId , AuthModule)
    {
        ...
        var sCheckMethod = EC$('input[name="check_method"]:checked').val();
        ...
        if (bResult == true) EC$('#findIdForm').submit();
 ...
 return false;//each 문에서는 break 안됨
    ...
    this.checkMobile = function()
    {
        ...
        EC$('input[name*="mobile"]').each(function(i, ele){
            if (EC$(this).val() == '') {
    ...
    this.displayElementByInput = function()
    {
        ...
        EC$.each(aInput, function(i, val) {
            if (EC$('.xans-member').find('#'+val).length < 1 && EC$('.xans-member').find('#'+val+'_view').length > 0) {
                EC$('.xans-member').find('#'+val+'_view').hide();
            }
        });
    };
...
function ipinFindIdPopup( sMallId )
{
    ...
    if ( EC$('#ipinFindIdForm').html() == null ) EC$('body').append(sipinFindIdForm);
    
    EC$('#ipinFindIdForm').attr('target', 'popupIpinfindid');
    EC$('#ipinFindIdForm').submit();
}
...
EC$(function(){
    ...
    if (SHOP.getLanguage() == 'ko_KR') {
        if (EC$('#login_id_type').val() === 'email') {
            EC$('#check_method1').hide();
            EC$("label[for='check_method1']").hide();
            bCheck3.click();
        }
    }
