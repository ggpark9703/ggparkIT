...
var BOARD_READ = {
     ...
     * @param iBoardNo
     */
    article_delete : function(sFormName, iBoardNo)
    {
        if (iBoardNo == '9') {
            var sMsg = __('해당 1:1문의글을 완전 삭제하시겠습니까?');
        } else {
            var sMsg = __('정말로 삭제 하시겠습니까?');
        }
        ...
        EC$('#bdf_action_type').val('delete');
        EC$('#'+sFormName).attr('action', EC$('#bdf_del_url').val()).submit();
    },
     ...
     * @param iBoardNo
     */
    article_modify : function(sFormName, iBoardNo)
    {
        var sMsg = __('정말로 수정 하시겠습니까?');
        ...
        EC$('#bdf_action_type').val('modify');
        EC$('#'+sFormName).attr('action', EC$('#bdf_modify_url').val()).submit();
    },
     ...
     * @param link
     */
    file_view : function (link)
    {
        //window.open(link, '', 'toolbar=no location=no scrollbars=yes resizable=yes width=680 height=780');
        location.href = link;
    },
