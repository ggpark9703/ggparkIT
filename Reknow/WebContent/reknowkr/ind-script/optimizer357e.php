...
var open_popup = function(aData) {
    ...
    this.win_popup = function() {
        try {
            //ECHOSTING-385143
            var popup = window.open(sUri, 'popup_' + aData.idx, 'width=' + aSize[0] + ', height=' + aSize[1] + ', top=' + aPos[0] + ', left=' + aPos[1] + ', toolbar=0, menubar=0, scrollbars=yes');
            popup.focus();
        } catch (e) {
...
var POPUP_UPDATE_EVENT_GUIDE = {
    openPopup : function ()
    {
        if (POPUP_UPDATE_EVENT_GUIDE.getCookie('UPDATEEVENT_LAYER_NOT_TODAY') === 'T') {
            ...
            EC$.get('/member/update_event.html','',function(sHtml)
            {
                if (bBuyLayer == true) {
