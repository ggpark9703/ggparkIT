...
function setFormType( type ) {
    ...
    try {
        ...
        var search_type_check = EC$(EC$('input[name=check_method]').get(3));
        ...
        var check_method = EC$("[id^='check_method']");
        for (var i = 0; i < check_method.length; i++) {
            if (EC$("[id^='check_method']")[i].style.display != 'none') {
                EC$('.xans-member').find('#check_method' + i).click();
                return;
            }
        }
...
{    
    ...
    this.action = function(sMallId, AuthModule)
    {
        ...
        if (SHOP.getLanguage() == 'ko_KR') {
            sCheckMethod = EC$('input[name="check_method"]:checked').val();
        } else {
            sCheckMethod = '2';
        }
        ...
        if (aFunctionMap[sCheckMethod] != 'SSN') {
            EC$('input[name*="ssn"]').each(function(i, ele){
                EC$(this).val('');
            });
        }
        ...
        // 초대 메일 발송 대상인지 확인
        var isDisplayActivationBtn = checkActivationMailTarget();
        if (isDisplayActivationBtn === true) {
            // submit 안함
            return false;
        }
         
        if (bResult == true) EC$('#findPasswdForm').submit();
    ...
    this.checkEmail = function()
    {
        ...
        EC$("form#findPasswdForm :input[name='name']").each(function(){
            if (EC$(this).val() == '') {
                alert( sprintf(__('%s을 입력하세요.'), searchTypeName));
                EC$(this).focus();
                bResult = false;
    ...
    this.checkMobile = function()
    {
        ...
        EC$("form#findIdForm :input[name*='name']").each(function(){
            if (EC$(this).val() == '') {
                alert( sprintf(__('%s을 입력하세요.'), searchTypeName));
                ele.focus();
                bResult = false;
        ...
        EC$('.xans-member-findpasswd input[name*="mobile"]').each(function(i, ele){
            if (EC$(this).val() == '') {
...
function ipinFindPwPopup( sMallId )
{
    ...
    if ( EC$('#ipinFindPwForm').html() == null ) EC$('body').append(sipinFindPwForm);

    EC$('#ipinFindPwForm').attr('target', 'popupIpinfindpw');
    EC$('#param2').val( document.findPasswdForm.member_id.value );

    EC$('#ipinFindPwForm').submit();
}
 ...
 * 초대 메일 발송 대상인지 확인
 */
function checkActivationMailTarget()
{
    // 계정 활성화 메일 버튼 노출 조건을 충족하는지 확인
    if (EC_FRONT_JS_CONFIG_MEMBER.bIsDisplayActivationSendButton !== true) {
        return false;
    }
    ...
    EC$.ajax({
        ...
        success: function(response) {
            ...
            if (response['passed'] !== true
                ...
                var sMsg = __('REQUEST.MEMBER.JOIN', 'MEMBER.FIND.FASSWD');
                var sSummary = '<p class="summary">' + sMsg + '</p>';

                var sButtonName = __('SENDING.ACTIVATION.MAIL', 'MEMBER.JOIN.CHECK.EMAIL');
                var sMemberInvitaionBuntton = '' +
                    '<div class="ec-base-button gColumn">' +
                        '<button id="send_mail_activation_btn" class="btnSubmit sizeL" type="button" onclick="sendAccountActivationMail(\'' + sMemberId + '\');">'+ sButtonName +'</button>' +
                    '</div>';

                var sMemberInvitaionDOM = sSummary + sMemberInvitaionBuntton;

                EC$('#emailMsg').html(sMemberInvitaionDOM);
            } else {
                // 활성화 계정이면 비밀번호 찾기
                EC$('#findPasswdForm').submit();
                return;
            }
        }
    });
    ...
    EC$.ajax({
        ...
        success : function(res) {
            ...
            if (sendResult.passed === true && sendResult.code === '0000'){
                alert(__('ACTIVATION.MAIL.SENDED', 'MEMBER.FIND.FASSWD'));
                location.href = '/';
            } else if (sendResult.passed === false && sendResult.code === '0001') {
                alert(__('ACTIVATION.MAIL.FAIL', 'MEMBER.FIND.FASSWD'));
            } else if (sendResult.passed === false && sendResult.code === '0002') {
                alert(__('ACTIVATION.MAIL.RESEND.VALID', 'MEMBER.FIND.FASSWD'));
            }
        }
    });
}
...
EC$(function(){
    ...
    var bSearchType = EC$('.xans-member').find('#searchType');

    try{
        // 유형 추가 여부 체크 유형 추가 안된 몰의 경우 예외 처리 구간에서 주민번호 가림.
        if (document.findPasswdForm.searchType.length > 1) {
            if (EC$(bSearchType).val() == null) EC$(bSearchType).val('indi');
            setFormType( EC$(bSearchType).val() );
        }
    }
    catch(e){ setFormType( 'indi' ); }

    EC$(bSearchType).change(function() {
        setFormType( EC$(this).val() );
    });
    


    //toggle
    var bCheck1 = EC$('.xans-member').find('#check_method0');
    var bCheck2 = EC$('.xans-member').find('#check_method1');
    var bCheck3 = EC$('.xans-member').find('#check_method2');
    var bCheck4 = EC$('.xans-member').find('#check_method3');
    var bCheck5 = EC$('.xans-member').find('#check_method4');

    var bCheck3Label = EC$('.xans-member').find('#search_type_mobile_lable');
    var mobile1 = EC$('.xans-member').find('#mobile1');
    
    if ( EC$('input[name*="mobile1"]').length < 1 ) {
        bCheck3.hide();
        bCheck3Label.hide();        
    }    

    EC$(bCheck1).click(function() {
        
        if (EC$(this).prop('checked') == true) {
            EC$('#email').val('');
            EC$('.xans-member').find('.mName').show();
            EC$('.xans-member').find('.ssn_no').show();
            EC$('.xans-member').find('.email').hide();
            EC$('.xans-member').find('.mobile').hide();
            EC$('.xans-member').find('.mobileauth').hide();                
            EC$('.xans-member').find('.ipin').hide();
        }
    });
    

    EC$(bCheck2).click(function() {

        if (EC$(this).prop('checked') == true) {
            EC$('#ssn1, #ssn2').val('');
            EC$('.xans-member').find('.mName').show();
            EC$('.xans-member').find('.email').show();
            EC$('.xans-member').find('.ssn_no').hide();
            EC$('.xans-member').find('.mobile').hide();
            EC$('.xans-member').find('.mobileauth').hide();                
            EC$('.xans-member').find('.ipin').hide();
        }       
    ...
    EC$(bCheck3).click(function() {
        ...
        if (EC$(this).prop('checked') == true) {
            ...
            EC$('.xans-member').find('.email').hide();
            EC$('.xans-member').find('.ssn_no').hide();
            EC$('.xans-member').find('.mobile').show();
            EC$('.xans-member').find('.mobileauth').hide();                
            EC$('.xans-member').find('.ipin').hide();                
        }       
    });
    ...
    EC$(bCheck4).click(function() {
        ...
        if (EC$(this).prop('checked') == true) {
            ...
            EC$('#email').val('');
            EC$('#mName').val('');
            EC$('.xans-member').find('.mName').hide();
            EC$('.xans-member').find('.email').hide();
            EC$('.xans-member').find('.ssn_no').hide();
            EC$('.xans-member').find('.mobile').hide();
            ...
            if ( EC$(this).val() == '4' ) {
               EC$('.xans-member').find('.mobileauth').hide(); 
               EC$('.xans-member').find('.ipin').show(); 
            }

            if ( EC$(this).val() == '5' ) {
               EC$('.xans-member').find('.mobileauth').show();
               EC$('.xans-member').find('.ipin').hide();
            }
        }     
    });

    EC$(bCheck5).click(function() {
