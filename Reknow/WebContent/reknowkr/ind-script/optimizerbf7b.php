...
EC$(function(){
    EC$("#boardWriteForm").submit(function(e){
        return submitForm.write("boardWriteForm");
    });

    // 운영자이고..
    if (is_admin == 'T') {
        EC$(window).on('beforeunload', function() {
            // 다음 상태가 처리해제 라는건 현 상태가 처리중.. 이라는 것..!!
            if (EC$('#eReplyStatusChangeTarget').val() == 'N') {
                return '다른 페이지로 이동하시면 답변상태가 처리중으로 남아있습니다.\n변경을 원하시면 처리해제를 눌러주세요.';
            }
        });

        // 본문에 focus 가도록 설정
        EC$('#content_IFRAME').contents().find('body').focus();
    }
    BOARD_WRITE.product_link();

    // 이벤트 발생시에만 게시안함 유무처리
    var $isPost = EC$("input:checkbox[name='is_post[]']");
    if ($isPost.length > 0) {
        $isPost.on('click', function(e){
            if (EC$(this).is(":checked")) {
                EC$("#is_post_checked").val('B');
            } else {
                EC$("#is_post_checked").val('F');
            }
        });
    }
});
...
var BOARD_WRITE = {
    ...
    {
        //플로알라 에디터 일경우
        // getContentFromFroala 함수에 플로알라 codeview 활성화 상태를 닫아 데이터를 적용하는 로직이 내포되어있음. 정상적인 데이터 삽입을 위해서 호출해줌.
        if (typeof EC_FROALA_ID != 'undefined') getContentFromFroala();
        ...
        if (parseInt(EC$('#depth').val()) >= 10) {
            alert(__('답변을 등록할 수 없습니다. (답변 차수 제한)'));
             return false;
        }

        if (EC$('#privacy_agreement_radio0').length > 0) {
            var target = EC$('#privacy_agreement_radio0')[0].checked;
            if (target  === false) {
        ...
        if (EC$('#ucc').length > 0 && EC$('#ucc').val() != '') {
            if ((/^http[s]?:\/\/[a-z0-9_\-]+(\.[a-z0-9_\-]+)+/i).test(EC$('#ucc').val()) === false) {
        ...
        // ECHOSTING-396990
        if (BOARD_WRITE.isAttatchedFirstFile() === false) {
        ...
        if (this.is_submitting === false) {
            ...
            EC$('#boardWriteForm input[name="attach_file[]"]').each(function() {
                if (EC$(this).val().length > 0) {
                    bExistAttachFile = true;
                    return false;
                }
            });
            // 첨부파일이 없다면 form > enctype 기본 속성값으로 셋팅
            if (bExistAttachFile == false && EC$('input[name^="delfile"]:checked').length < 1) {
                EC$('#boardWriteForm').attr('enctype', 'application/x-www-form-urlencoded');
            }

            EC$(window).off('beforeunload');
    ...
    product_link: function() {
        var $a = EC$('#iPrdImg').parent('a');
        if ($a.length) {
            $a.on('click', function(e){
                var product_no = 0;
                var url = EC$(this).attr('href');
                EC$.each(url.substr(url.indexOf('?')+1).split('&'), function(i, str) {
    ...
    /**
     * 첨부파일을 하나라도 등록할경우, 첨부파일1(첫번째 file input)이 첨부되어있는지의 여부 반환.
     * @returns boolean 유효성여부
     */
    isAttatchedFirstFile: function() {
        var aAttatchFiles = EC$("input[name='attach_file[]']");
        for (var i = 0; i < aAttatchFiles.length; i++) {
            var oFileElment = aAttatchFiles[i];
            if (oFileElment.files.length > 0) {
                if (aAttatchFiles[0].files.length <= 0) {
                    return false;
                }
            }
        }
